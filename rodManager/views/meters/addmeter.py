...
def add_meter(request):
    ...
    if "rodManager.manageMeters" not in request.user.get_all_permissions():
        return Response(
            {"error": "You don't have permission to add meters."},
            status=status.HTTP_403_FORBIDDEN,
        )
    if (not request.data["address"] and not request.data["garden"]) or not request.data[
        "type"
    ]:
        return Response(
            {"error": "Address or garden,  and type are required."},
