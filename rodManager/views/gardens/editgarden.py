...
def edit_garden(request):
    if "rodManager.manageGardens" not in request.user.get_all_permissions():
        return Response(
            {"error": "You don't have permission to edit gardens."},
            status=status.HTTP_403_FORBIDDEN,
        )
    if not request.data["id"]:
        return Response(
            {"error": "Garden ID is required."}, status=status.HTTP_400_BAD_REQUEST
        )
    if (
        not request.data["sector"]
        or not request.data["avenue"]
        or not request.data["number"]
        or not request.data["area"]
        or not request.data["status"]
    ):
        return Response(
    ...
    garden.save()
    return Response(
        {"success": "Garden edited successfully."}, status=status.HTTP_200_OK
    )
