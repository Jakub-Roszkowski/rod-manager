...
class FeeByIdView(APIView):
    @extend_schema(
        summary="Get fees",
        description="Get all fees in the system.",
    ...
    def get(self, request, billing_period_id):
        ...
        leaseFees = Fee.objects.filter(
            fee_type="Lease", billing_period=billing_period_id
        )
        utilityFees = Fee.objects.filter(
            fee_type="Utility", billing_period=billing_period_id
        )
        additionalFees = Fee.objects.filter(
            fee_type="Additional", billing_period=billing_period_id
        )
